...
router.get('/menu/:restaurantSlug', async (req, res) => {
  ...
  // TODO: const items = await MenuItem.find({ restaurantId: restaurant?.id, isAvailable: true })
...
router.post('/auth/login', async (req, res) => {
  ...
  // TODO: const payload = { id: user._id, name: user.name, role: user.role, restaurantId: user.restaurantId, exp: Date.now() + 24*60*60*1000 }
...
router.get('/restaurant/:id/tables', async (req, res) => {
  ...
  // TODO: const tables = await Table.find({ restaurantId: id })
...
router.get('/restaurant/:id/orders', async (req, res) => {
  // TODO: const { id } = req.params
  // TODO: const orders = await Order.find({ restaurantId: id })
  // TODO:   .populate('items')
...
router.put('/menu-items/:id/price', async (req, res) => {
  // TODO: const { id } = req.params
  // TODO: const { newPrice } = req.body
  // TODO: if (req.user.role !== 'owner')
  // TODO:   return res.status(403).json({ error: 'Forbidden' })
  // TODO: const item = await MenuItem.findByIdAndUpdate(id, { price: newPrice }, { new: true })
  // TODO: res.json({ success: true, data: item })
  res.json({ success: true, data: null });
});

router.get('/restaurant/:id/popular-items', async (req, res) => {
  // TODO: const { id } = req.params
  // TODO: const items = await MenuItem.find({ restaurantId: id })
